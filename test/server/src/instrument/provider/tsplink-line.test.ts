...
import * as Namespace from '../../../../../server/src/instrument/provider/tsplink-line'

@suite class TsplinkLineTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Tsplink-Line to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Tsplink-Line'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Tsplink-Line'
        )
    }
}
