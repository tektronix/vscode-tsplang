...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger'
import { emptySpec } from '../emptySpec'

@suite class TriggerTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Trigger to export signatures'
        )
    }
