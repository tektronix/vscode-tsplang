...
import * as Namespace from '../../../../../server/src/instrument/provider/dataqueue'
import { emptySpec } from '../emptySpec'

@suite class DataqueueTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Dataqueue to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Dataqueue'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Dataqueue to export signatures'
        )
    }
