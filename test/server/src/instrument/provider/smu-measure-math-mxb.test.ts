...
import * as Namespace from '../../../../../server/src/instrument/provider/smu-measure-math-mxb'

@suite class SmuMeasureMathMxbTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Smu-Measure-Math-Mxb to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Smu-Measure-Math-Mxb'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Smu-Measure-Math-Mxb'
        )
    }
}
