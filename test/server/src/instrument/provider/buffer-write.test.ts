...
import * as Namespace from '../../../../../server/src/instrument/provider/buffer-write'
import { emptySpec } from '../emptySpec'

@suite class BufferWriteTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Buffer-Write to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Buffer-Write'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Buffer-Write to export signatures'
        )
    }
