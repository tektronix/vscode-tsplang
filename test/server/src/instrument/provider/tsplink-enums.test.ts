...
import * as Namespace from '../../../../../server/src/instrument/provider/tsplink-enums'

@suite class TsplinkEnumsTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Tsplink-Enums to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Tsplink-Enums'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Tsplink-Enums'
        )
    }
}
