...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger-enums'

@suite class TriggerEnumsTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger-Enums to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger-Enums'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Trigger-Enums'
        )
    }
}
