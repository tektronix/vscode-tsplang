...
import * as Namespace from '../../../../../server/src/instrument/provider/smu-measure-limit-low'

@suite class SmuMeasureLimitLowTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Smu-Measure-Limit-Low to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Smu-Measure-Limit-Low'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Smu-Measure-Limit-Low'
        )
    }
}
