...
import * as Namespace from '../../../../../server/src/instrument/provider/reset'
import { emptySpec } from '../emptySpec'

@suite class ResetTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Reset to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Reset'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Reset to export signatures'
        )
    }
