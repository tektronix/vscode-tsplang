...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger-model'
import { emptySpec } from '../emptySpec'

@suite class TriggerModelTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger-Model to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger-Model'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Trigger-Model to export signatures'
        )
    }
