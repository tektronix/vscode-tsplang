...
import * as Namespace from '../../../../../server/src/instrument/provider/eventlog-enums'

@suite class EventlogEnumsTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Eventlog-Enums to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Eventlog-Enums'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Eventlog-Enums'
        )
    }
}
