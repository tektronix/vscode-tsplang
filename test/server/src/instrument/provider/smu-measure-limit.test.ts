...
import * as Namespace from '../../../../../server/src/instrument/provider/smu-measure-limit'

@suite class SmuMeasureLimitTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Smu-Measure-Limit to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Smu-Measure-Limit'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Smu-Measure-Limit'
        )
    }
}
