...
import * as Namespace from '../../../../../server/src/instrument/provider/file'
import { emptySpec } from '../emptySpec'

@suite class FileTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected File to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from File'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected File to export signatures'
        )
    }
