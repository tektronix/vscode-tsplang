...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger-timer'
import { emptySpec } from '../emptySpec'

@suite class TriggerTimerTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger-Timer to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger-Timer'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Trigger-Timer to export signatures'
        )
    }
