...
import * as Namespace from '../../../../../server/src/instrument/provider/status-enums'

@suite class StatusEnumsTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Status-Enums to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Status-Enums'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Status-Enums'
        )
    }
}
