...
import * as Namespace from '../../../../../server/src/instrument/provider/coroutine'
import { emptySpec } from '../emptySpec'

@suite class CoroutineTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Coroutine to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Coroutine'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Coroutine to export signatures'
        )
    }
