...
import * as Namespace from '../../../../../server/src/instrument/provider/smu-measure-math'

@suite class SmuMeasureMathTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Smu-Measure-Math to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Smu-Measure-Math'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Smu-Measure-Math'
        )
    }
}
