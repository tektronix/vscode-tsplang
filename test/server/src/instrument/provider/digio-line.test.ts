...
import * as Namespace from '../../../../../server/src/instrument/provider/digio-line'

@suite class DigioLineTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Digio-Line to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Digio-Line'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Digio-Line'
        )
    }
}
