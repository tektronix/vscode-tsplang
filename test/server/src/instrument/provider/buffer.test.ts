...
import * as Namespace from '../../../../../server/src/instrument/provider/buffer'
import { emptySpec } from '../emptySpec'

@suite class BufferTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Buffer to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Buffer'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Buffer to export signatures'
        )
    }
