...
import * as Namespace from '../../../../../server/src/instrument/provider/digio'
import { emptySpec } from '../emptySpec'

@suite class DigioTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Digio to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Digio'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Digio to export signatures'
        )
    }
