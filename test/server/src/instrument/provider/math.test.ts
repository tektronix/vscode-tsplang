...
import * as Namespace from '../../../../../server/src/instrument/provider/math'
import { emptySpec } from '../emptySpec'

@suite class MathTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Math to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Math'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Math to export signatures'
        )
    }
