...
import * as Namespace from '../../../../../server/src/instrument/provider/table'
import { emptySpec } from '../emptySpec'

@suite class TableTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Table to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Table'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Table to export signatures'
        )
    }
