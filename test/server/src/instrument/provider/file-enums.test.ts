...
import * as Namespace from '../../../../../server/src/instrument/provider/file-enums'

@suite class FileEnumsTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected File-Enums to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from File-Enums'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from File-Enums'
        )
    }
}
