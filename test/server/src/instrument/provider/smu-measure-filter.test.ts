...
import * as Namespace from '../../../../../server/src/instrument/provider/smu-measure-filter'

@suite class SmuMeasureFilterTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Smu-Measure-Filter to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Smu-Measure-Filter'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Smu-Measure-Filter'
        )
    }
}
