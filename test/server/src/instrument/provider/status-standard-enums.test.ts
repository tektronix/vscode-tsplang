...
import * as Namespace from '../../../../../server/src/instrument/provider/status-standard-enums'

@suite class StatusStandardEnumsTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Status-Standard-Enums to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Status-Standard-Enums'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Status-Standard-Enums'
        )
    }
}
