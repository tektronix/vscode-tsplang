...
import * as Namespace from '../../../../../server/src/instrument/provider/delay'
import { emptySpec } from '../emptySpec'

@suite class DelayTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Delay to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Delay'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Delay to export signatures'
        )
    }
