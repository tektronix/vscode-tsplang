...
import * as Namespace from '../../../../../server/src/instrument/provider/bufferVar'

@suite class BufferVarTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected BufferVar to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from BufferVar'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from BufferVar'
        )
    }
}
