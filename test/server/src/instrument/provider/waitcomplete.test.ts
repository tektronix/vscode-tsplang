...
import * as Namespace from '../../../../../server/src/instrument/provider/waitcomplete'
import { emptySpec } from '../emptySpec'

@suite class WaitcompleteTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Waitcomplete to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Waitcomplete'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Waitcomplete to export signatures'
        )
    }
