...
import * as Namespace from '../../../../../server/src/instrument/provider/tsplink'
import { emptySpec } from '../emptySpec'

@suite class TsplinkTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Tsplink to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Tsplink'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Tsplink to export signatures'
        )
    }
