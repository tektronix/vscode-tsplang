...
import * as Namespace from '../../../../../server/src/instrument/provider/localnode'
import { emptySpec } from '../emptySpec'

@suite class LocalnodeTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Localnode to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Localnode'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Localnode to export signatures'
        )
    }
