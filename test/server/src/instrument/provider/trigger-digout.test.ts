...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger-digout'

@suite class TriggerDigoutTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger-Digout to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger-Digout'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Trigger-Digout'
        )
    }
}
