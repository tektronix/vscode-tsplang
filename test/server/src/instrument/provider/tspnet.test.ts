...
import * as Namespace from '../../../../../server/src/instrument/provider/tspnet'
import { emptySpec } from '../emptySpec'

@suite class TspnetTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Tspnet to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Tspnet'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Tspnet to export signatures'
        )
    }
