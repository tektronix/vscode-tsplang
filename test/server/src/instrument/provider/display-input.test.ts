...
import * as Namespace from '../../../../../server/src/instrument/provider/display-input'
import { emptySpec } from '../emptySpec'

@suite class DisplayInputTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Display-Input to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Display-Input'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Display-Input to export signatures'
        )
    }
