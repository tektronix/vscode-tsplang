...
import * as Namespace from '../../../../../server/src/instrument/provider/display'
import { emptySpec } from '../emptySpec'

@suite class DisplayTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Display to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Display'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Display to export signatures'
        )
    }
