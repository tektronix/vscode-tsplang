...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger-timer-start'

@suite class TriggerTimerStartTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger-Timer-Start to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger-Timer-Start'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Trigger-Timer-Start'
        )
    }
}
