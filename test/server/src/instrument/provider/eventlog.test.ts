...
import * as Namespace from '../../../../../server/src/instrument/provider/eventlog'
import { emptySpec } from '../emptySpec'

@suite class EventlogTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Eventlog to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Eventlog'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Expected Eventlog to export signatures'
        )
    }
