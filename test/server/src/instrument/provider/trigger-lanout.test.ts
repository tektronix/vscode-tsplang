...
import * as Namespace from '../../../../../server/src/instrument/provider/trigger-lanout'

@suite class TriggerLanoutTest {
    @test('Exports completions')
    exportsCompletions(): void {
        assert(
            Namespace.completions !== undefined,
            'Expected Trigger-Lanout to export completions'
        )
    }
    ...
    exportsNoCompletionDocs(): void {
        assert(
            ...
            'Unexpected completionDocs export from Trigger-Lanout'
        )
    }
    ...
    exportsSignatures(): void {
        assert(
            ...
            'Unexpected signatures export from Trigger-Lanout'
        )
    }
}
