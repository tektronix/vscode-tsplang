...
const tspnetCompletions: Array<CompletionItem> = [
    ...
    },
    {
        ...
        documentation: {
            kind: MarkupKind.Markdown,
            value: 'This command connects a device to another device through the LAN interface. If the portNumber is 23, the interface uses the Telnet protocol and sets appropriate termination characters to communicate with the device.\n\nIf a portNumber and initString are provided, it is assumed that the remote device is not TSP‑enabled. The Model 2460 does not perform any extra processing, prompt handling, error handling, or sending of commands. In addition, the tspnet.tsp.* commands cannot be used on devices that are not TSP‑enabled.\n\nIf neither a portNumber nor an initString is provided, the remote device is assumed to be a Keithley Instruments TSP‑enabled device. Depending on the state of the tspnet.tsp.abortonconnect attribute, the Model 2460 sends an abort command to the remote device on connection.\n\nThe Model 2460 also enables TSP prompts on the remote device and event management. The Model 2460 places remote errors and events from the TSP‑enabled device in its own event queue and prefaces these events with Remote Error, followed by an event description.\n\nDo not manually change either the prompt functionality (localnode.prompts) or show events by changing localnode.showerrors or localnode.showevents on the remote TSP-enabled device. If you do this, subsequent tspnet.tsp.* commands using the connection may fail.\n\nYou can simultaneously connect to a maximum of 32 remote devices.'
        },
        kind: CompletionItemKind.Function,
    ...
    },
    {
        ...
        documentation: {
            kind: MarkupKind.Markdown,
            value: 'This command sends a command string to the remote instrument. A termination is added to the command string when it is sent to the remote instrument (tspnet.termination()). You can also specify a format string, which causes the command to wait for a response from the remote instrument. The Model 2460 decodes the response message according to the format specified in the format string and returns the message as return values from the function (see tspnet.read() for format specifiers).\n\nWhen this command is sent to a TSP-enabled instrument, the Model 2460 suspends operation until a timeout error is generated or until the instrument responds. The TSP prompt from the remote instrument is read and discarded. The Model 2460 places any remotely generated errors and events into its event queue. When the optional format string is not specified, this command is equivalent to tspnet.write(), except that a termination is automatically added to the end of the command.'
        },
        kind: CompletionItemKind.Function,
