...
const triggerCompletions: Array<CompletionItem> = [
    ...
    },
    {
        ...
        documentation: {
            kind: MarkupKind.Markdown,
            value: 'The measure block must occur in the trigger model before the branch‑on‑dynamic‑limits block. If no measure block is defined, the measurement from the previous measure block is used. If no previous measure block exists, an error is reported.'
        },
        kind: CompletionItemKind.Function,
    ...
    },
    {
        ...
        documentation: {
            kind: MarkupKind.Markdown,
            value: 'Each measure function can have up to 5 unique user delay times (M1 to M5). Each source function can also have up to 5 unique user delay times (S1 to S5). The delay time is set by the user‑delay command, which is only available over a remote interface.\n\nThough the trigger model can be used with any function, the user delay is set per function. Make sure you are setting the delay for the function you intend to use with the trigger model.'
        },
        kind: CompletionItemKind.Function,
...
const triggerSignatures: Array<SignatureInformation> = [
    ...
    SignatureInformation.create(
        'trigger.model.load() — Config List(measureConfigList, sourceConfigList, delay, bufferName)',
        undefined,
        ParameterInformation.create(
            'measureConfigList',
    ...
    SignatureInformation.create(
        'trigger.model.load() — Duration Loop(duration, delay, bufferName)',
        undefined,
        ParameterInformation.create(
            'duration',
