...
const statusCompletions: Array<CompletionItem> = [
    {
        detail: 'This function clears event registers.',
        documentation: {
    ...
    },
    {
        ...
        documentation: {
            kind: MarkupKind.Markdown,
            value: 'You can use this command to read the status byte, which is returned as a numeric value.\n\nWhen an enabled status event occurs, a summary bit is set in this register to indicate the event occurrence. The returned value can indicate that one or more status events occurred. If  more than one bit of the register is set, statusByte equals the sum of their decimal weights. For example, if 129 is returned, bits B0 and B7 are set (1 + 128).\n\nThe meanings of the individual bits of this register are shown in the following table.'
        },
    ...
    {
        detail: 'This function allows you to map events to bits in the Operation Event Register.',
        documentation: {
            kind: MarkupKind.Markdown,
