...
export async function getTriggerTimerCommandSet(cmds: Array<ApiSpec>): Promise<CommandSet> {
    return new Promise<CommandSet>((
        ...
        try {
            ...
            cmds.forEach((cmd: ApiSpec) => {
                triggerTimerCompletions.forEach((completion: CompletionItem) => {
                    if (cmd.label.localeCompare(resolveCompletionNamespace(completion)) === 0) {
                        resultCompletions.push(completion)
                    }
                })

                triggerTimerSignatures.forEach((signature: SignatureInformation) => {
