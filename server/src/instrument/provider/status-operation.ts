...
export async function getStatusOperationCommandSet(cmds: Array<ApiSpec>): Promise<CommandSet> {
    return new Promise<CommandSet>((
        ...
        try {
            ...
            cmds.forEach((cmd: ApiSpec) => {
                statusOperationCompletions.forEach((completion: CompletionItem) => {
                    if (cmd.label.localeCompare(resolveCompletionNamespace(completion)) === 0) {
                        resultCompletions.push(completion)
                    }
                })

                statusOperationSignatures.forEach((signature: SignatureInformation) => {
