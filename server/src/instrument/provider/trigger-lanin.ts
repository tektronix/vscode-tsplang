...
export async function getTriggerLaninCommandSet(cmds: Array<ApiSpec>): Promise<CommandSet> {
    return new Promise<CommandSet>((
        ...
        try {
            ...
            cmds.forEach((cmd: ApiSpec) => {
                triggerLaninCompletions.forEach((completion: CompletionItem) => {
                    if (cmd.label.localeCompare(resolveCompletionNamespace(completion)) === 0) {
                        resultCompletions.push(completion)
                    }
                })

                triggerLaninSignatures.forEach((signature: SignatureInformation) => {
