...
export async function getTsplinkCommandSet(cmds: Array<ApiSpec>): Promise<CommandSet> {
    return new Promise<CommandSet>((
        ...
        try {
            ...
            cmds.forEach((cmd: ApiSpec) => {
                tsplinkCompletions.forEach((completion: CompletionItem) => {
                    if (cmd.label.localeCompare(resolveCompletionNamespace(completion)) === 0) {
                        resultCompletions.push(completion)
                    }
                })

                tsplinkSignatures.forEach((signature: SignatureInformation) => {
